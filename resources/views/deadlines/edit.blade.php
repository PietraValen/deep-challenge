...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-primary mb-1">Editar Prazo</h2>
            <p class="text-muted small mb-0">Atualize as informações do prazo</p>
        </div>
                    ...
                    <form action="{{ route('deadlines.update', $deadline) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                ...
                                id="title" name="title" value="{{ old('title', $deadline->title) }}" required autofocus>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <select
                                ...
                                @foreach($processes as $process)
                                    <option value="{{ $process->id }}" {{ old('process_id', $deadline->process_id) == $process->id ? 'selected' : '' }}>{{ $process->title }} ({{ $process->number ?? 'S/N' }})</option>
                                @endforeach
                            </select>
                            @error('process_id')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local"
                                    ...
                                    value="{{ old('due_date', $deadline->due_date->format('Y-m-d\TH:i')) }}" required>
                                @error('due_date')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <select
                                    ...
                                    <option value="Pendente" {{ old('status', $deadline->status) == 'Pendente' ? 'selected' : '' }}>Pendente</option>
                                    <option value="Concluído" {{ old('status', $deadline->status) == 'Concluído' ? 'selected' : '' }}>Concluído</option>
                                </select>
                                @error('status')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <textarea
                                ...
                                rows="4">{{ old('description', $deadline->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
